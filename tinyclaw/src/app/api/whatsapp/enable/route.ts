...
export async function POST() {
  ...
  if (!user.vpsPassword || user.vpsPassword === "DevPassword123!") {
    await db
      .update(users)
      .set({ whatsappConnected: "configured" })
      .where(eq(users.clerkId, userId));
    return NextResponse.json({ success: true, message: "[DEV] WhatsApp channel enabled" });
  }
...
"`
      ...
      if (!healthOk) {
        ...
        console.error("[WHATSAPP] Gateway failed to restart:", logs);
      ...
      return NextResponse.json({
        success: true,
        message: "WhatsApp channel enabled! Open WebChat and run the login command to scan QR code.",
      });
...
" 2>&1`
    ...
    return NextResponse.json({ error: "Failed to enable WhatsApp. Please try again." }, { status: 500 });
  }
}
